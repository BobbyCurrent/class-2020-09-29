...
d_2019 <- read_excel("raw_data/class_enrollment_summary_by_term_12.20.19.xlsx",
  ...
  filter(! is.na(course_title)) %>%
   select( -instructor_full_name, -course_section_code) %>%
  rename(id = course_id,
         title = course_title,
         name = course_name,
...
d_2019_take_3 <- read_enrollment(file = "raw_data/class_enrollment_summary_by_term_12.20.19.xlsx")
...
bind_rows(d_2019_take_3, d_2015, d_2016, d_2017, d_2018, .id = "year")
...
```{r sc5}
bind_rows(d_2019_take_3, d_2015, d_2016, d_2017, d_2018, .id = "year") %>%
  filter( department == "Economics" | department == "Government" | department == "Computer Science") %>%
